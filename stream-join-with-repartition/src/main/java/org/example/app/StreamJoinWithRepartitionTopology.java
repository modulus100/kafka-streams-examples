...
@Configuration
public class StreamJoinWithRepartitionTopology {
    ...
    public KStream<String, CheckoutConfirmed> checkoutJoinStream(StreamsBuilder builder) {
        ...
        KStream<String, CheckoutConfirmed> joined = payments.join(
                ...
                StreamJoined.<String, PaymentAuthorized, InventoryReserved>as("payments-inventory-join")
                        .withKeySerde(Serdes.String())
                        .withValueSerde(paymentSerde)
                        .withOtherValueSerde(inventorySerde)
        );
    ...
    private static <T> JacksonJsonSerde jsonSerde(Class<T> targetType) {
        return new JacksonJsonSerde(targetType)
                .ignoreTypeHeaders()
                .noTypeInfo();
    }
}
